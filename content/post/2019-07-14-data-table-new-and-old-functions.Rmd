...
This is the first post looking at the **data.table** presentations from **UseR!2019** which was held in Toulouse from 2019-07-07-2019-10-10. For starters we are going to take a look at the presentations from *Arun Srinivasan*, one of the main developers of **data.table**. His talk is called **THE #RDATATABLEPACKAGE
...
```
When you pass a column to the `by` argument, internally the number of the unique values for that column gets evaluated. Afterwards the rows gets associated to the respective group. Then the expression provided in `j`, `sum(valA)` gets evaluated for the groups. 
...
`.SD` stands for *Subset of Data* and contains all the columns, except for the grouping columns supplied in `by`, for each group.
...
Now we create a `data.table` with 20 columns and 50 million rows to demonstrate that expression in `i`can run in parallel.
...
Recently the radix order has been parallelised. We define a new data set.
...
github: - <https://github.com/Rdatatable>
